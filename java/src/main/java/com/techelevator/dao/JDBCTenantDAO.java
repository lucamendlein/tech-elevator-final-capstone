...
public class JDBCTenantDAO implements TenantDAO{
    ...
    @Override
    public void approveTenant(Tenant tenant, String approveTenant) {
        String sql = "update tenant set approve_tenant = ? where tenant_id = ? ";
        jdbcTemplate.update(sql, tenant.getApprovalStatus() , tenant.getTenantId());


    }

    @Override
    public void addWorkOrder(Tenant tenant, String workOrder) {
        String sql = "update tenant set work_order = ? where tenant_id = ?";
        jdbcTemplate.update(sql, workOrder, tenant.getTenantId());
    }
